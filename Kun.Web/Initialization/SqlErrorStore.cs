...
namespace Kun
{
    ...
    public sealed class SqlErrorStore : ErrorStore
    {
        ...
        protected override Task<bool> ProtectErrorAsync(Guid guid)
        {
            ...
            {
                return Task.FromResult(new SqlUpdate("Exceptions")
        ...
        protected override Task<bool> ProtectErrorsAsync(IEnumerable<Guid> guids)
        {
            using (var c = GetConnection())
            {
                return Task.FromResult(new SqlUpdate("Exceptions")
        ...
        protected override Task<bool> DeleteErrorsAsync(IEnumerable<Guid> guids)
        {
            using (var c = GetConnection())
            {
                return Task.FromResult(new SqlUpdate("Exceptions")
        ...
        /// <returns>True if the error was found and deleted, false otherwise</returns>
        protected override Task<bool> HardDeleteErrorAsync(Guid guid)
        {
            using (var c = GetConnection())
            {
                return Task.FromResult(new SqlDelete("Exceptions")
                    .Where(
                        new Criteria("[GUID]") == guid &
            ...
            using (var c = GetConnection())
            {
                return Task.FromResult(new SqlUpdate("Exceptions")
                    ...
                    .Where(
                        ...
                        new Criteria("[ApplicationName]") == (applicationName.IsNullOrEmpty() ? ApplicationName : applicationName))
                    .Execute(c, ExpectedRows.Ignore) > 0);
            }
        }
        ...
        protected override bool LogError(Error error)
        {
            using (var c = GetConnection())
            {
                if (Settings.RollupPeriod.HasValue && error.ErrorHash.HasValue)
                {
                    if (isSqlServer)
                    {
                        var queryParams = new Dapper.DynamicParameters(new
                        {
                            error.DuplicateCount,
                            error.ErrorHash,
                            ApplicationName = error.ApplicationName.Truncate(50),
                            minDate = DateTime.UtcNow.Add(Settings.RollupPeriod.Value.Negate())
                        });
    ...
    Where Id In (Select Top 1 Id
                ...
                Where ErrorHash = @ErrorHash
                    ...
                    else
                    {
                        ...
                        update.SetParam("@minDate", DateTime.UtcNow.Subtract(Settings.RollupPeriod.Value));
                        ...
                        if (count > 0)
                        {
                            ...
                            q.SetParam("@minDate", DateTime.UtcNow.Subtract(Settings.RollupPeriod.Value));
...
Select * 
 ...
 Where [GUID] = @guid", new { guid }).FirstOrDefault(); // a guid won't collide, but the AppName is for security
        ...
        protected override Task<int> GetErrorCountAsync(DateTime? since = null, string applicationName = null)
        {
            using (var c = GetConnection())
            {
                return Task.FromResult(c.Query<int>(
                    new SqlQuery()
                        .Dialect(c.GetDialect())
                        .From("Exceptions")
                        .Select("Count(*)")
